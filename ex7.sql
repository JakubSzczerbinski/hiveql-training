...
CREATE OR REPLACE VIEW salaries_pln_company_rank AS
    SELECT
        *,
        COUNT(*) OVER (PARTITION BY company_id ORDER BY paycheck_pln DESC) as salary_rank
    FROM salaries_pln;

SELECT company_id, paycheck_pln
FROM salaries_pln_company_rank
WHERE salary_rank <= 3
LIMIT 20;
GROUP BY company_id;
